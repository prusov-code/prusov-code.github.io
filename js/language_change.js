$('.english_language').on('click',function () {
	if($('.intro>h3').text()=='Разработка, ведение и продвижение сайтов') {
		...
		setTimeout(function() {
		    ...
		    $('.contacts>h3').text('Interested in my services? Contact me using the details below.');
		    $('.contacts>h3').slideDown();
		     ...
		     $('.about_me .text_block h1').text('Nikita Prusov');
		     ...
		     $('.about_me .text_block p').eq(2).slideDown();
 		     $('.about_me .text_block p').eq(3).text('I continue to maintain my projects after the completion of the main work, solving routine issues with the domain, hosting, as well as minor edits on the site.');
		     ...
		     $('.principles .items .item').eq(1).find('.title').text('Responsiveness');
		     $('.principles .items .item').eq(1).find('.regular').text('Websites look great on any device — from a computer screen to a smartphone.');
		     $('.principles .items .item').eq(2).find('.title').text('Performance');
		     $('.principles .items .item').eq(2).find('.regular').text('Fast site loading time and interaction without delay is my highest priority.');
		     $('.principles .items .item').eq(3).find('.title').text('SEO optimization');
		     $('.principles .items .item').eq(3).find('.regular').text('My sites have a high-quality semantic core, which positively affects search engine rankings, as well as the influx of target audience to the site.');
            $('.principles .items .item').eq(0).find('.title').slideDown();
		     ...
		     $('#technical_part>h3').eq(1).text('Frameworks, libraries and other technologies:');
		     $('#technical_part>h3').eq(2).text('Databases:');
 		     $('#technical_part>h3').eq(3).text('Additional technologies:');
 		     $('#technical_part>h3').eq(4).text('Summary:');
     	     for(var i=0;i<4;i++) {
			     $('#technical_part>h3').eq(i).slideDown();
		     }
		       $('.summary>a>p').text('Download resume');
...
$('.russian_language').on('click',function () {
	if($('.intro>h3').text()!='Разработка, ведение и продвижение сайтов') {
		...
		setTimeout(function() {
		    ...
		    $('.contacts>h1').slideDown();
		    $('.contacts>h3').text('Интересуют услуги? Свяжитесь со мной по контактам ниже.');
		    $('.contacts>h3').slideDown();
		     ...
		     $('.principles .items .item').eq(0).find('.regular').text('Перед началом работы я всегда лично общаюсь с заказчиком, что помогает лучше понять продукт и выбрать правильную стратегию. Каждый проект уникален и разрабатывается с нуля с учётом всех пожеланий заказчика.');
		     $('.principles .items .item').eq(1).find('.title').text('Адаптивность');
		     $('.principles .items .item').eq(1).find('.regular').text('Сайты отлично смотрятся на любых устройствах — от экрана компьютера до смартфона.');
            ...
            $('.principles .items .item').eq(0).find('.title').slideDown();
		     ...
		     $('#technical_part>h3').eq(2).text('Базы данных:');
 		     $('#technical_part>h3').eq(3).text('Дополнительные технологии:');
